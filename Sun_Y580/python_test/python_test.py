...
aliens = [alien_00, alien_01, alien_02]  # 列表里有字典
print(aliens)
print('==' * 50)
active = True
# while active:
...
user_profile = build_profile('albert', 'einstein', location='princeton', field='physics')  # 以字典格式输出

print(user_profile)

with open('test.txt', 'r', encoding='UTF-8') as test:  # 打开文件格式需要注意
    # A = test.read()
    # print(A)
    print('===' * 50)
    B = test.readlines()  # readlines()获取文件中所有数据，每行数据以\n换行符结尾，数据以列表方式存储；
    # readline()获取文件中第一行数据
    print(B)
    print('==' * 50)

with open('test_write.txt', 'w')as W:
    write_test = W.write('sun quan wei')
with open('test_write.txt', 'r')as R:
    read_test = R.readlines()
    print(read_test)
...
#     print(S)
try:
    with open("Alice's Adventures in Wonderland - Lewis Carroll.txt", 'r')as Alice:
        temp = Alice.read()
except FileNotFoundError:
    print('路径不正确或者文件不存在')
else:
    word = temp.split()  # 将文章分割为以单词为单位的列表
    print(word)
    num_word = len(word)
    print(num_word)
print('\\\\' * 50)
...
#JSON文件读取、写入
filename = 'Alice_s Adventures in Wonderland_Lewis Carroll.json'
with open(filename, 'r')as Alice_json:
    temp_json = Alice_json.readlines()
    print(temp_json)

numbers=[2,3,4,5,6,7,8]

with open('nambers.json','w')as f_obj:
